...
void extractParam(std::string& line, std::string& param) {
	...
	if (start != std::string::npos && end != std::string::npos && end > start) {
		...
		// Remove spaces from the param string
...
void writeTXT(const std::string &path) {
	...
	if (fmodel.is_open()) {
		...
		while (std::getline(fmodel, line)) {
            ...
            if (line.find(key_amb_NN) != std::string::npos && line.find("//") == std::string::npos && line.find(key_amb_bypassNN1) == std::string::npos && line.find(key_amb_bypassNN2) == std::string::npos) {
	...
	if (load_nn_model_src == "LoadFromFlash" && (model_name_ac.length() != 0 && model_name_ac != "NA_MODEL") && (model_name_od != "NA_MODEL" || model_name_fd != "NA_MODEL" || model_name_fr != "NA_MODEL" || model_name_ic != "NA_MODEL")) {
		if (PRINT_DEBUG) std::cout << "[" << __LINE__ << "] model_name_ac: " << model_name_ac << std::endl;
